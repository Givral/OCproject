...
cap = cv2.VideoCapture("E:/omnivid/bomni-5840 (1)/scenario1/side-3.mp4")
subtractorKNN = cv2.createBackgroundSubtractorKNN()
book = load_workbook('dataside3.xlsx')
while True:
    for i in range(2, 1000, 1):
        ...
        opening_m = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel1, iterations=1)
        closing_m = cv2.morphologyEx(opening_m, cv2.MORPH_CLOSE, kernel2, iterations=3)

        _, contours, _ = cv2.findContours(closing_m4, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        for c in contours:
                ...
                if area > 300:
                    ...
                    data = pandas.DataFrame(
                        {'ratio_w_h': [ratio], 'area': [area], 'area_rect': [area_rect], 'ratio_area': [ratio_area],
                         'angle_of_rect': [angle], 'angle_o_line': [angle_line]})
                    writer = pandas.ExcelWriter('dataside0.xlsx', engine='openpyxl')
                    writer.book = book
